        ...
        ItemTouchHelper helper = new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                if (position >= 0 && position < infoListAdapter.getItemsList().size()) {
                    InfoItem item = infoListAdapter.getItemsList().get(position);

                    if (item instanceof StreamInfoItem) {
                        StreamInfoItem streamItem = (StreamInfoItem) item;
                        if (direction == ItemTouchHelper.LEFT) {
                            // Queue (Enqueue)
                            NavigationHelper.enqueueOnPlayer(requireContext(), new SinglePlayQueue(streamItem));
                        } else {
                            // Play Next
                            NavigationHelper.enqueueNextOnPlayer(requireContext(), new SinglePlayQueue(streamItem));
                        }
                    }
                }
